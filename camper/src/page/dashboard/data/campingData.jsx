...
import  { CampingAddModal, CaravanAddModal } from '../modal';
...
const handleEdit = async (oldname) => {
    try {
        const response = await axios.put(`http://localhost:5500/camping/${oldname}`, { newName, newCompany, newLicense, newPassanger, newCost, newType, newDate, newRating, newLocation});
        setNewName("");
        fetchData();
    } catch (error) {
...
const handleDelete = async (name) => {
    try {
        const response = await axios.delete(`http://localhost:5500/camping/${name}`)
    } catch (error) {
        console.error(error)
    }
}
                    ...
                    {data.map((value, index)=>{
                        return(
                            <TableRow key={index}>
                                ...
                                <td>{/* <img src={value.photo} alt="" width={50}/> */}<div>Photo</div></td>
                                <td>{value.name}</td>
